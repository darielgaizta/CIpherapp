...
def encrypt(p, k, toBase64=False):
 randomized_key = random_key(k.decode())
 vigenere_key = extended_vigenere_encrypt(randomized_key, k.decode()).encode()

 j = 0
 s = key_scheduling(k) # KSA key

 for j in range(256):
  s[j] = s[j] ^ vigenere_key[j % len(vigenere_key)]
